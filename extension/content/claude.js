...
const SELECTORS = {
  turnWrapper: '[data-testid="user-message"], [data-testid="assistant-message"]',
...
function getContainerId(turnElement) {
  return turnElement.getAttribute('data-testid') + '-' + Math.random().toString(36).slice(2, 6);
}
...
function processUserMessage(turnElement) {
  ...
  sendMessage('User', null, content, messageId, attachments);
}
...
function setupObserver() {
  ...
  const chatContainer = document.querySelector('[data-testid="chat-container"], [role="log"], main');
  if (!chatContainer) {
...
function detectProject() {
  ...
  const instructionsEl = document.querySelector('[data-testid="project-instructions"], .prose:not(.font-claude-message)');
  const instructions = instructionsEl ? instructionsEl.textContent.trim() : null;
